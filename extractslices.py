...
def extractslices():
    ...
    current = os.getcwd()
    masks_path = (r'E:/brain segemetation/novel patch-wise Unet/IBSR_Dataset'+"/"+"Masks"+"/")        
    masks = glob.glob(masks_path+'*_ana.nii')
    
    image_path = (r'E:/brain segemetation/novel patch-wise Unet/IBSR_Dataset'+"/"+"Raw"+"/")        
    images = glob.glob(image_path+'*_strip.nii')
    ...
    for f in masks[0:subjects]:
        ...
        for j in range(21,196,1): 
            ROI = Mask_data[:,:,j].T
            ROI = np.pad(ROI,((64,64),(0,0)),'constant', constant_values=(0, 0))
            ROI = label2class(ROI)
            maskdatas[i] = ROI
            i += 1
    ...
    for f in images[0:subjects]:
        ...
        for j in range(21,196,1): # extract slices starting from 10 and ending at 152 with interval of 3 slices in between
            Selected = image_data[:,:,j] 
            ROI = Selected.T        
            ROI = np.pad(ROI,((64,64),(0,0)),'constant', constant_values=(0, 0)) # zero-padding to make the dimensions 256 x 256
